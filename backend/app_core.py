# app_core.py - النسخة المحدثة للتوافق مع Hugging Face Spaces
...
# ==================== مسار مجلد geodata الديناميكي ====================
# يقرأ المجلد مباشرة من داخل مستودع المشروع في السيرفر
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
GEO_DATA_PATH = os.path.join(BASE_DIR, "backend", "geodata")
...
async def generate_report(request: AnalyzeRequest):
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    body {{ font-family: 'Segoe UI', 'Arial', sans-serif; background: white; color: #1e293b; line-height: 1.5; }}
    .report {{ max-width: 100%; }}
    .header {{ text-align: center; border-bottom: 3px solid #064e3b; padding-bottom: 15px; margin-bottom: 20px; }}
    .logo {{ font-size: 24px; font-weight: bold; color: #064e3b; }}
    .logo span {{ color: #10b981; }}
    .subtitle {{ font-size: 12px; color: #047857; margin-top: 5px; }}
    .report-title {{ font-size: 18px; font-weight: bold; color: #064e3b; margin: 15px 0 8px; }}
    .report-id {{ font-size: 10px; color: #64748b; }}
    .two-columns {{ display: flex; gap: 20px; margin-bottom: 20px; }}
    .column {{ flex: 1; }}
    .info-card {{ background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 15px; height: 100%; }}
    .card-title {{ font-size: 14px; font-weight: bold; color: #064e3b; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 12px; }}
    .info-table {{ width: 100%; border-collapse: collapse; }}
    .info-table td {{ padding: 6px 0; border-bottom: 1px solid #e2e8f0; font-size: 11px; }}
    .info-table td:first-child {{ width: 40%; font-weight: bold; color: #475569; }}
    .mini-map {{ background: linear-gradient(135deg, #e2e8f0 0%, #cbd5e1 100%); height: 160px; border-radius: 10px; display: flex; flex-direction: column; align-items: center; justify-content: center; font-size: 12px; color: #475569; }}
    .map-marker {{ font-size: 32px; margin-bottom: 8px; }}
    .score-card {{ background: {color}; border-radius: 16px; padding: 25px; text-align: center; margin-bottom: 20px; }}
    .score-value {{ font-size: 52px; font-weight: bold; color: white; }}
    .score-label {{ font-size: 14px; color: rgba(255,255,255,0.9); margin-top: 8px; }}
    .results-table {{ width: 100%; border-collapse: collapse; margin-bottom: 20px; }}
    .results-table th {{ background: #064e3b; color: white; padding: 10px; font-size: 12px; }}
    .results-table td {{ border: 1px solid #e2e8f0; padding: 10px; text-align: center; font-size: 12px; }}
    .recommendation-box {{ background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 15px; margin-bottom: 20px; }}
    .recommendation-title {{ font-weight: bold; margin-bottom: 8px; color: #064e3b; }}
    .qr-section {{ text-align: center; padding: 15px; background: #f8fafc; border-radius: 12px; margin-bottom: 20px; }}
    .footer {{ text-align: center; font-size: 9px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 15px; margin-top: 10px; }}
    @media print {{ body {{ margin: 0; padding: 0; }} .no-print {{ display: none; }} }}
    .print-btn {{ background: #064e3b; color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 12px; margin-bottom: 15px; display: inline-block; }}
